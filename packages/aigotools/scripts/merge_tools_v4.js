/**
 * merge_tools_v4.js
 * 按行读取 seed-data.ts，在 SEED_SITES 数组闭合 ]; 前插入新工具
 * 补全所有缺失字段，正确处理枚举
 */
...
function mapState(v) {
  ...
  if (s === "published") return "SiteState.published";
...
function mapProcessStage(v) {
  ...
  if (s === "failed") return "ProcessStage.failed";
...
/** 将单个工具对象转为 TypeScript 文本（补全所有缺失字段） */
function siteToTs(obj) {
  const has = (k) => obj[k] !== undefined && obj[k] !== null;

  const _id = has("_id") ? obj._id : ("site-" + (obj.siteKey || obj.name || "unknown").toLowerCase().replace(/[^a-z0-9]+/g, "-"));
...
function merge() {
  ...
  // 找 export function getSeedStats 的行号
  let statsLine = -1;
  for (let i = 0; i < lines.length; i++) {
    if (lines[i].includes("export function getSeedStats")) {
      statsLine = i;
      break;
    }
  }
  if (statsLine === -1) throw new Error("找不到 export function getSeedStats");

  // 往前找 ];
  let closeLine = -1;
  for (let i = statsLine - 1; i >= 0; i--) {
    if (lines[i].trim() === "];") {
      closeLine = i;
      break;
    }
  }
  if (closeLine === -1) throw new Error("找不到 SEED_SITES 的 ]; 闭合行");

  console.log(`在行 ${closeLine + 1} (];) 之前插入 ${tools.length} 个工具`);

  // 构建新条目文本（末尾加逗号，因为是在 ]; 前插入）
  const newEntries = tools.map(t => siteToTs(t)).join(",\n") + ",";

  // 在 closeLine 处插入（]; 行会被保留）
  lines.splice(closeLine, 0, newEntries);

  const newContent = lines.join("\n");
  fs.writeFileSync(SEED_FILE, newContent, "utf8");
  console.log(`✅ 完成！文件大小: ${newContent.length} 字节`);
}
