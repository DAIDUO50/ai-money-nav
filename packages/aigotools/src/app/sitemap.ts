...
export async function generateSitemaps() {
  try {
    ...
    console.warn("Sitemap database unavailable, falling back to base sitemap.", error);
    return [{ id: -1 }];
  }
}
...
export default async function sitemap({ id }: { id: number }) {
  ...
  if (id === -1) {
    ...
    try {
      ...
      console.warn("Sitemap database unavailable, skipping site entries for this sitemap page.", error);
    }
  }
