...
export async function getFeaturedSites(size = 12) {
  try {
    ...
    return result.length ? result : getSeedFeaturedSites(size);
  } catch (error) {
    console.log("Get featured sites", error);
  }

  return getSeedFeaturedSites(size);
}
...
export async function getLatestSites(size = 12) {
  try {
    ...
    return result.length ? result : getSeedLatestSites(size);
  } catch (error) {
...
export async function getSiteMetadata(siteKey: string) {
  try {
    ...
    if (!site) {
      const seedSite = getSeedSiteByKey(siteKey);

      if (!seedSite) return null;

      return {
        title: seedSite.name,
        description: seedSite.metaDescription || seedSite.description,
        keywords: seedSite.metaKeywords,
      };
    }
...
export async function getSiteDetailByKey(siteKey: string) {
  try {
    ...
    if (!site) {
      ...
      const suggests = SEED_SITES.filter(
        (s) => s.siteKey !== siteKey && s.categories.some((c) => seedSite.categories.includes(c)),
      ).slice(0, 8);

      return { site: seedSite, suggests };
    }
...
export async function getFeaturedCategories() {
  try {
    ...
    const result = categories.map(categoryToObject);

    return result.length ? result : getSeedFeaturedCategories();
  } catch (error) {
    console.log("Get featured categories", error);
  }

  return getSeedFeaturedCategories();
}
