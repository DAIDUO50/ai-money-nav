...
// If no MongoDB URI configured, the app runs in seed-data-only mode
// (no database required for basic site listing display)
...
async function dbConnect() {
  if (!AppConfig.mongoUri) {
    console.warn("MongoDB not configured - using seed data fallback");
    return null;
  }
  if (cached.conn) {
    return cached.conn;
